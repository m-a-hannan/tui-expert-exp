...
import de.reckendrees.systems.tui.expert.managers.notifications.KeeperService;
import de.reckendrees.systems.tui.expert.managers.notifications.NotificationManager;
import de.reckendrees.systems.tui.expert.managers.notifications.NotificationMonitorService;
import de.reckendrees.systems.tui.expert.managers.notifications.NotificationService;
...
import de.reckendrees.systems.tui.expert.managers.xml.options.Notifications;
...
import de.reckendrees.systems.tui.expert.tuils.Assist;
import de.reckendrees.systems.tui.expert.tuils.CustomExceptionHandler;
...
import de.reckendrees.systems.tui.expert.tuils.PublicIOReceiver;
import de.reckendrees.systems.tui.expert.tuils.SimpleMutableEntry;
import de.reckendrees.systems.tui.expert.tuils.Tuils;
import de.reckendrees.systems.tui.expert.tuils.interfaces.Inputable;
import de.reckendrees.systems.tui.expert.tuils.interfaces.Outputable;
...
public class LauncherActivity extends AppCompatActivity implements Reloadable {
    ...
    private void finishOnCreate() {
        ...
        if (useSystemWP) {
            setTheme(de.reckendrees.systems.tui.expert.R.style.Custom_SystemWP);
        } else {
            setTheme(de.reckendrees.systems.tui.expert.R.style.Custom_Solid);
        }
                ...
                try {
                    ...
                    if (!Tuils.hasNotificationAccess(this)) {
                        ...
                        if (i.resolveActivity(getPackageManager()) == null) {
                            Toast.makeText(this, de.reckendrees.systems.tui.expert.R.string.no_notification_access, Toast.LENGTH_LONG).show();
                        } else {
                            startActivity(i);
                        }
                    }
                    ...
                    intent.putExtra(PrivateIOReceiver.TEXT, getString(de.reckendrees.systems.tui.expert.R.string.output_notification_error) + Tuils.SPACE + er.toString());
                }
            } else {
                Tuils.sendOutput(Color.RED, this, de.reckendrees.systems.tui.expert.R.string.notification_low_api);
            }
        }
        ...
        setContentView(de.reckendrees.systems.tui.expert.R.layout.base_view);
        ...
        ViewGroup mainView = (ViewGroup) findViewById(de.reckendrees.systems.tui.expert.R.id.mainview);
...
//        }
    ...
    public void onCreateContextMenu(ContextMenu menu, View v, ContextMenu.ContextMenuInfo menuInfo) {
        ...
        suggestion = (SuggestionsManager.Suggestion) v.getTag(de.reckendrees.systems.tui.expert.R.id.suggestion_id);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if(requestCode == TUIXT_REQUEST && resultCode != 0) {
            if(resultCode == TuixtActivity.BACK_PRESSED) {
                Tuils.sendOutput(this, de.reckendrees.systems.tui.expert.R.string.tuixt_back_pressed);
            } else {
            ...
            switch (requestCode) {
                case COMMAND_REQUEST_PERMISSION:
                    if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                        ...
                        ui.setOutput(getString(de.reckendrees.systems.tui.expert.R.string.output_nopermissions), TerminalManager.CATEGORY_OUTPUT);
                ...
                case STARTING_PERMISSION:
                    ...
                    while(count < permissions.length && count < grantResults.length) {
                        if(grantResults[count] == PackageManager.PERMISSION_DENIED) {
                            Toast.makeText(this, de.reckendrees.systems.tui.expert.R.string.permissions_toast, Toast.LENGTH_LONG).show();
                            new Thread() {
                                @Override
                                public void run() {
                                    super.run();
